...
> on
  ...
  ECSBase<T>
{
  late final hookOnDisposeKey = ECSHookKey<void Function(E self)>(
    'IsDisposable', 'onDispose'
  );

  Iterable<void Function(E self)> get _onDisposeFns
    => hooksOf(hookOnDisposeKey);

  /// Registers [fn] to be called at the disposal of the owned ECS object.
  @nonVirtual
  E listenOnDispose(void Function(E self) fn) {
    addHook(hookOnDisposeKey, fn);
    return self;
  }
